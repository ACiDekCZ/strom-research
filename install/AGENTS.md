...
1. Check whether strom is installed: run `strom --version`.
...
4. Change the research ONLY through `strom` commands, never by editing its
   files. Ask the user before decisions: whom to research, what they know,
   anything that needs their permission. When strom opens a window asking
   for consent, tell the user to answer it — you cannot.
