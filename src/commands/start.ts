...
import { chromiumBrowser, openFileWith, openInBrowser, revealFile } from "../core/chromium.ts";
...
import { writeGedcoms, writeTreeGed } from "./output.ts";
...
register({
  ...
  run: async (ctx: Context, { opts }) => {
    ...
    const agentHere = isAgent(ctx.env);
    ...
    const base = {
      ...
      shared,
      ...(agent === "claude" ? { chrome: browserConnectors(ctx.env, shared).length > 0 } : {}),
    };
    ...
    for (const s of abandonedSessions(Tree.open(tree.root, ctx.env))) {
      ...
      commitNow(t, `${s.id} interrupted: its conversation had ended`);
    }
    ...
    const env = { ...prependPath(ctx.env, shimDir(tree)), STROM_WORKER: worker };
    let code: number;
    try {
...
register(
  {
    ...
    description:
      ...
      `Otherwise the results are ready for it in output/tree-strom.ged (in the app: Import). Web: ${STROM_APP_URL}`,
    options: [{ name: "live", type: "boolean", description: "the app follows this research while it goes on (the live bridge)" }],
    examples: ["strom app", "strom app --live", "strom app install"],
    run(ctx, { opts }) {
      const lang = ctx.uiLang();
      const root = appOpensResearch(ctx.env) ? ctx.locateTree() : undefined;
      if (root) {
        ...
        // The file as the research is now: the installed app takes it, or the user drags it in.
        const file = writeTreeGed(tree, { for: "strom", stromVersion: ctx.settings.stromVersion(tree.config) }).file;
        // B, D: the app reaches strom on this computer — in a Chromium browser, never whatever is the default (Safari cannot).
        ...
        if (browser) {
          ...
          const info = startLive(root, ctx.env);
          if (!info) throw new StromError("the bridge did not start", { hint: "strom live serve shows why" });
          const url = follow ? liveAppUrl(info.url, ctx.env) : importAppUrl(`${info.url}/tree.ged`, ctx.env);
          const opened = openInBrowser(browser, url, ctx.env);
          const text = opened ? ui(lang, follow ? "ui.app.following" : "ui.app.research", { name, where: browser.name }) : ui(lang, "ui.app.url", { url });
          return { text, data: { opened, via: "browser", browser: browser.name, url, bridge: info.url, follow } };
        }
        // A: no such browser to hand it over, but the Strom app installed from one takes the file itself (its
        // file handler) — not when strom is pointed at another copy of the app (STROM_APP_URL: a beta, a development one).
        const installed = installedStromApp(ctx.env);
        if (!follow && !ctx.env.STROM_APP_URL && installed && /Chrome|Edge|Brave|Chromium|Vivaldi/.test(installed.kind) && openFileWith(installed.path, file, ctx.env))
          return { text: ui(lang, "ui.app.research", { name, where: installed.kind }), data: { opened: true, via: "file", file, app: installed.path, follow } };
        // C: no such browser — the app opens, the file is shown, the user drags it into the app's window.
        openForUser(installed?.path ?? stromAppUrl(ctx.env), ctx.env);
        const shown = revealFile(file, ctx.env);
        return {
          text: lines(follow ? ui(lang, "ui.app.nolive") : undefined, ui(lang, "ui.app.drag", { file: ctx.display(file), shown: shown ? ui(lang, "ui.app.drag.shown") : "" })),
      ...
      if (opts.live) {
        ...
        if (!info) throw new StromError("the bridge did not start", { hint: "strom live serve shows why" });
        const url = liveAppUrl(info.url, ctx.env);
      ...
      const app = noticeStromApp(ctx.settings, ctx.env, { look: true }) ?? installedStromApp(ctx.env);
      const opened = openForUser(app?.path ?? stromAppUrl(ctx.env), ctx.env);
      const text = opened ? ui(lang, "ui.app.opened", { where: app ? app.kind : ui(lang, "ui.app.browser") }) : ui(lang, "ui.app.url", { url: stromAppUrl(ctx.env) });
      return { text, data: { opened, url: stromAppUrl(ctx.env), ...(app ? { installed: app } : {}) } };
    },
  },
  {
    path: ["live"],
    ...
    run(ctx) {
      ...
      return info
        ? { text: lines(`the bridge runs: ${info.url}`, `  the Strom app following it: ${liveAppUrl(info.url, ctx.env)}`, "  stop: strom live stop"), data: { running: true, ...info, app: liveAppUrl(info.url, ctx.env) } }
  ...
  },
  {
    ...
    run(ctx) {
      ...
      if (!info) throw new StromError("the bridge did not start", { hint: "strom live serve shows why" });
      return { text: lines(`the bridge runs: ${info.url}`, `  the Strom app following it: ${liveAppUrl(info.url, ctx.env)}`), data: { ...info, app: liveAppUrl(info.url, ctx.env) } };
    },
  },
  {
    path: ["live", "stop"],
  ...
  },
  {
    ...
    run(ctx) {
      const lang = ctx.uiLang();
      const app = installedStromApp(ctx.env);
      if (app) {
      ...
      const opened = openForUser(stromAppUrl(ctx.env), ctx.env);
      return { text: opened ? ui(lang, "ui.app.install") : ui(lang, "ui.app.url", { url: stromAppUrl(ctx.env) }), data: { installed: false, opened, url: stromAppUrl(ctx.env) } };
    },
  },
);
